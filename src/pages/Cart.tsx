...
const Cart = () => {
  ...
  useEffect(() => {
    const totals = cartProduct.reduce(
      (sum, product) => {
        // sum.regular += product.totalCost;
        sum.regular += product?.discountedPrice * product?.quantity;
        sum.discounted += (product?.totalCost || product.discountedPrice) * product?.quantity;
        return sum;
      },
    ...
    <Container>
      {cartProduct?.length > 0 ? (
        <>
          ...
          <div className="mt-10 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
            ...
            >
              ...
              <Link to="/checkout">
                <button
                  className='uppercase my-8 w-full bg-black hover:bg-secondary text-white font-bold py-4 rounded focus:outline-none focus:shadow-outline'
                  type='button'
                >
                  Proceed to checkout
                </button>
        ...
        <div className="bg-white h-96 flex flex-col gap-2 items-center justify-center py-5 rounded-lg border border-gray-200 drop-shadow-2xl">
          ...
          <p className="text-lg max-w-[600px] my-10 text-center text-gray-600 tracking-wide leading-6">
            Your cart is empty. 
          </p>
          <Link to="/product">
